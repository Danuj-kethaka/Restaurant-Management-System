        ...
        <div className="hidden md:flex items-center gap-5">
          ...
          {currentUser ? (
            <>
              ...
              <Link to={currentUser.role === "admin" ? "/admindashboard" : "/useraccount"} className="bg-amber-500 hover:bg-amber-400 transition text-black px-6 py-3 rounded-lg font-semibold text-sm tracking-wider shadow-lg shadow-amber-500/30">
                My Account
              </Link>
            </>
          ) : (
            <>
              ...
              <Link to="/register" className="bg-amber-500 hover:bg-amber-400 transition text-black px-6 py-3 rounded-lg font-semibold text-sm tracking-wider shadow-lg shadow-amber-500/30">REGISTER</Link>
            </>
          )}
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-black text-white">
        ...
        <div className="max-w-3xl mx-auto text-center pt-48 pb-20 px-6">
          ...
          <Link to="/" className="inline-block bg-amber-500 hover:bg-amber-400 transition text-black px-8 py-3 rounded-lg font-semibold text-sm tracking-wider shadow-lg shadow-amber-500/30">
            BROWSE MENU
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto pt-32 pb-20 px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-2/3 space-y-4">
            {cart.map((item) => (
              <div key={item._id} className="bg-zinc-900 border border-white/10 rounded-2xl p-4 shadow-sm">
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <div className="flex-1 min-w-[140px]">
                    ...
                    <small className="text-zinc-400">${Number(item.price).toFixed(2)} each</small>
                  </div>

                  <div className="flex items-center border border-white/10 rounded-lg overflow-hidden">
                    <button className="px-3 py-2 text-white hover:bg-white/10 transition" onClick={() => updateQuantity(item._id, item.quantity - 1)}>
                      <Minus size={14} />
                    </button>
                    <span className="px-4 py-2 text-center bg-black/40 min-w-[3rem]">{item.quantity}</span>
                    <button className="px-3 py-2 text-white hover:bg-white/10 transition" onClick={() => updateQuantity(item._id, item.quantity + 1)}>
                      <Plus size={14} />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-4 ml-auto">
                    ...
                    <button className="text-zinc-400 hover:text-red-500 transition" onClick={() => removeFromCart(item._id)}>
                      <Trash2 size={18} />
                    </button>
                  </div>
                </div>
          ...
          {/* Order Summary Sidebar - FIXED BUTTON */}
          <div className="w-full lg:w-1/3">
            <div className="bg-zinc-900 border border-white/10 rounded-2xl p-6 sticky top-24">
              ...
              <button 
                ...
                className="w-full bg-amber-500 hover:bg-amber-400 transition text-black py-3 rounded-lg font-semibold text-sm tracking-wider shadow-lg shadow-amber-500/30"
              >
                Proceed to Checkout
              </button>
            </div>
